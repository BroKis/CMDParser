...
TEST_CASE("Parse all arguments")
{
    ...
    std::unique_ptr<args_types::Arg> arg4 = std::make_unique<args_types::StringArg>(helpArg);
    ...
    SECTION("Correct parse all argument")
    {
        ...
        parser.add(std::move(arg4));

        const char** args = new const char* [2];
        const auto result = parser.parse((int)argsv.size(), &argsv[0]).isOk();
        std::cout << std::boolalpha << result << std::endl;

        Arg* arg4 = parser.getArgumentFromVector(3);
        REQUIRE(arg4->isDefined());
        std::cout << parser.GetHelp() << std::endl;
        Arg* arg1 = parser.getArgumentFromVector(0);
        REQUIRE(arg1->isDefined()); 
        Arg* arg2 = parser.getArgumentFromVector(1);
        REQUIRE(arg2->isDefined());
        Arg* arg3 = parser.getArgumentFromVector(2);
